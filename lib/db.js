...
// In-memory fallback database is strictly restricted to development only
let memDbInstance = null;
function getMemDb() {
  if (isProduction) {
    throw new Error('FATAL: In-memory fallback database is strictly forbidden in production.');
  }
  if (!memDbInstance) {
    memDbInstance = require('./mem-db.js');
  }
