...
function getPool() {
  if (!pool) {
    pool = mysql.createPool({
      ...
      database: process.env.DB_NAME || 'digitax',
...
async function ensureInit() {
  if (initialized || connectionFailed) return;
  if (!initPromise) {
    initPromise = initDb()
      .then(() => {
        initialized = true;
      ...
      .catch((err) => {
        ...
        console.warn('Using in-memory fallback database (seeded with admin user admin123@gmail.com / 12345)\n');
      });
  }
...
const db = {
  ...
  async all(sql, params = []) {
    await ensureInit();
    if (useMemDb) return getMemDb().all(sql, params);
    try {
      const p = getPool();
      const [rows] = await p.execute(sql, params);
      return rows;
    } catch (err) {
      console.warn('❌ Database execute error (db.all):', err.message);
      return [];
    }
  },
  ...
  async get(sql, params = []) {
    await ensureInit();
    if (useMemDb) return getMemDb().get(sql, params);
    try {
      const p = getPool();
      const [rows] = await p.execute(sql, params);
      return rows[0] || null;
    } catch (err) {
      console.warn('❌ Database execute error (db.get):', err.message);
      return null;
    }
  },
  ...
  async run(sql, params = []) {
    await ensureInit();
    if (useMemDb) return getMemDb().run(sql, params);
    try {
      const p = getPool();
      const [result] = await p.execute(sql, params);
      return { insertId: result.insertId, affectedRows: result.affectedRows };
    } catch (err) {
      console.warn('❌ Database execute error (db.run):', err.message);
  ...
  async exec(sql) {
    await ensureInit();
    if (useMemDb) return getMemDb().exec(sql);
    try {
      const p = getPool();
      await p.query(sql);
    } catch (err) {
      console.warn('❌ Database query error (db.exec):', err.message);
    }
  },
};
